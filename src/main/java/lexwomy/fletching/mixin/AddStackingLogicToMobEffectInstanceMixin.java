...
public abstract class AddStackingLogicToMobEffectInstanceMixin {
  // Allows stacking effects to bypass normal time checks when adding hidden effects
  // This allows for deterioration and also allows the previous effect to be accessed to be
  // compounded
  ...
  // {
  //        boolean stacking = that.equals(Fletching.FRENZY); //TODO - Add longbow stacking behavior
  // later
  ...
  private boolean allowStackingToBypassAmplifierCheck(boolean original, MobEffectInstance that) {
    boolean stacking = that.is(FletchingEffects.FRENZY) || that.is(FletchingEffects.FOCUS);
    return original || stacking;
  }

  // Sets the current amplifier for the new effect to be the sum of the previous and the incoming
  // stacking effect
  @WrapOperation(
      method = "update",
      at =
  ...
  private void applyPreviousAmplifierToCurrent(
      MobEffectInstance instance, int value, Operation<Void> original, MobEffectInstance that) {
    if (that.is(FletchingEffects.FRENZY)) {
      int new_amplifier = Math.min(value + instance.getAmplifier() + 1, 39);
      original.call(instance, new_amplifier);
    } else if (that.is(FletchingEffects.FOCUS)) {
      int new_amplifier = Math.min(value + instance.getAmplifier() + 1, 2);
      original.call(instance, new_amplifier);
    } else {
